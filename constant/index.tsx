export const TREATMENT_DETAIL_CONTENTS = [
  {
    description: (
      <p>
        針治療は、特別な細い針を体の特定のツボに刺すことで、深いリラクゼーションをもたらしながら、
        <span>『 肩こり、腰痛、頭痛、ストレス関連の不調 』</span>
        を効果的に緩和します。
      </p>
    ),
    imagePath: "/images/treatment-hari.jpg",
    subTitle: "acupuncture",
    title: "鍼",
  },
  {
    description: (
      <p>
        お灸は、特別なモグサを体の特定のツボに焚き付けることで、心地良い温かさを感じながら、
        <span>『冷え性、疲労回復、免疫力の向上、生理痛』</span>
        などの症状を効果的に改善します。
      </p>
    ),
    imagePath: "/images/treatment-okyu.jpg",
    subTitle: "moxibustion",
    title: "灸",
  },
  {
    description: (
      <p>
        整体は、身体のバランスを整えるための手技を中心とした治療法で、自然な姿勢や動きを取り戻すことを目的とし、
        <span>『関節の痛み、筋肉のこりや緊張、身体の歪み』</span>
        などの不調を効果的に緩和します。
      </p>
    ),
    imagePath: "/images/treatment-seitai.jpg",
    subTitle: "manipulative",
    title: "整体",
  },
  {
    description: (
      <p>
        産後ケアは、出産後の身体や心の変化に対応し、母たちの健康と快適さをサポートする治療法です。特に
        <span>『骨盤の歪み、産後の腰痛、体型のリカバリー』</span>
        など、出産後に多くの女性が経験する症状や悩みを効果的にケアします。
      </p>
    ),
    imagePath: "/images/treatment-kotsuban.jpg",
    subTitle: "postpartum care",
    title: "産後ケア",
  },
  {
    description: (
      <p>
        吸い玉は、特別なカップを使用して皮膚を吸引する伝統的な治療法です。この吸引により、血行促進や筋肉のリラクゼーションを促し、
        <span>『疲労回復、筋肉のこりや緊張、冷え性』</span>
        などの症状を効果的に緩和します。
      </p>
    ),
    imagePath: "/images/treatment-suidama.jpg",
    subTitle: "sucking ball",
    title: "すい玉",
  },
]

export const QUESTIONS_CONTENTS = [
  {
    answer: (
      <pre>
        はい、当院は<span>完全予約制</span>
        です。LINEや電話で気軽にご予約くださいね！
      </pre>
    ),
    question: "予約は必要ですか？",
  },
  {
    answer: (
      <pre>
        通常、治療時間は<span>約60分</span>
        くらいを予定しています。お身体の状態に合わせて、最適なケアを提供いたしますので、ご安心くださいね！
      </pre>
    ),
    question: "治療時間はどのくらいですか？",
  },
  {
    answer: (
      <pre>
        良い質問ですね。実は当院で使用する針はとても細く、多くの方からは
        <span>『ちょっとしたチクリ感』</span>
        として、時には
        <span>『心地よい刺激』</span>
        として感じられることが多いんです。初めての方も、リラックスして気持ちの良い施術を体験いただけるよう、細心の注意を払っております。どうぞお楽しみに！
      </pre>
    ),
    question: "鍼治療って痛いですか？",
  },
  {
    answer: (
      <p>
        動きやすい服装、例えば
        <span>『Tシャツ、ジャージ』</span>
        がオススメです。
        <span>『きついジーンズ、タイトスカート』</span>
        は動きを制限する可能性があるので、できればご遠慮くださいませ。
      </p>
    ),
    question: "どんな服装で行けば良いですか？",
  },
  {
    answer: (
      <p>
        現在、当院では<span>現金のみ</span>
        のお支払いとなっております。ご理解のほど、よろしくお願い申し上げます。
      </p>
    ),
    question: "支払い方法は何がありますか？",
  },
  {
    answer: (
      <p>
        はい、当院には駐車場を
        <span>1台分</span>
        ご用意しています。もし利用される際は、事前にお知らせいただけると幸いです。
      </p>
    ),
    question: "駐車場はありますか？",
  },
]

export const CONSULTATION_HOURS = {
  afternoons: ["●", "●", "●", "●", "●", "ー"],
  mornings: ["ー", "ー", "ー", "ー", "ー", "●"],
  weeks: ["月", "火", "水", "木", "金", "土"],
}
